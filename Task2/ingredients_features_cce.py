# =============================================================================
# Task2a: Food101 Images Multiclass Classification using Ingredients with CCE
...
# Epochs 22
# Train loss: 2.460  Train acc.: 0.392  Top-5 acc.: 0.672  Test loss: 2.480  Test acc.: 0.390  Top-5 acc.: 0.667
# Training complete in 19m 35s
# Test Top-1 Accuracy = 0.3911881188118812
# Test Top-5 Accuracy = 0.6722277227722773
# 
# 
# Augmented Dataset
# 
...
class ingr2classClassifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(in_features=1488, out_features=128, bias=True)
        self.fc2 = nn.Linear(in_features=128, out_features=101, bias=True)
...
torch.save(task2_Classifier, os.path.join(output_data_dir, 'Task2/cce/' + type_of_dataset + '/model.pth'))

# Plot metrics
plt.plot(train_losses, label='Training loss')
plt.plot(test_losses, label='Validation loss')
plt.legend(frameon=False)
plt.savefig(os.path.join(output_data_dir, 'Task2/cce/' + type_of_dataset + '/losses.jpg'), transparent=True, dpi=200)
plt.show()

plt.plot(train_accuracies, label='Train Accuracy')
plt.plot(test_accuracies, label='Test Accuracy')
plt.legend(frameon=False)
plt.savefig(os.path.join(output_data_dir, 'Task2/cce/' + type_of_dataset + '/accuracies.jpg'), transparent=True, dpi=200)
plt.show()

plt.plot(train_top5_accuracies, label='Train Top-5 Accuracy')
plt.plot(test_top5_accuracies, label='Test Top-5 Accuracy')
plt.legend(frameon=False)
plt.savefig(os.path.join(output_data_dir, 'Task2/cce/' + type_of_dataset + '/top5_accuracies.jpg'), transparent=True, dpi=200)
plt.show()
...
# Load model
model = torch.load(os.path.join(output_data_dir, 'Task2/cce/' + type_of_dataset + '/model.pth'))
model = model.to(device)
model = task2_Classifier
